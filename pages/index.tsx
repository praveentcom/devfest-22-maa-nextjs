...
const AppStart: NextPage = () => {
    ...
    async function sendAgenda(event: React.SyntheticEvent) {
        ...
        if (!isApiCall) {
            ...
            try {
                var response = await axios.post("https://www.avgeek.io/api/utahjs/accept-request", {
                    email: target.email.value,
                    name: target.name.value,
                })
                if (response.status === 200) {
                    setIsRequestSuccess(true);
                    setTimeTaken(response.data.data.additional_data.time_taken)
                }
            } catch { } finally {
                setIsApiCall(false)
            }
